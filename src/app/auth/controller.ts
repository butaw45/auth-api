...
class Auth {
  ...
  async login(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const otherRefreshToken = cookie?.jwt 
      ...
      const currentUser = { ...user[0], refreshToken: [...otherRefreshToken,refreshToken] };
  ...
  async logout(req: Request, res: Response, next: NextFunction) {
    ...
    const otherUser = db.filter((item) => !item.refreshToken.includes(refreshToken));
    const otherRefreshToken = user[0].refreshToken.filter(item => item !== refreshToken)
